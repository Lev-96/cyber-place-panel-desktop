...
export class RealtimeService {
  ...
  private async tick(): Promise<void> {
    try {
      ...
      const [places, services, bookings] = await Promise.all([
      ...
      const snapshot = this.buildSnapshot(places, services, bookings);
      this.last = snapshot;
  ...
  private buildSnapshot(places: Place[], services: Service[], bookings: Booking[]): BranchSnapshot {
    const at = new Date();
    ...
    const placeSnapshots: PlaceSnapshot[] = places.map((p) => {
      ...
      return { place: p, status: p.computeStatus(list, at), bookings: list };
    });
