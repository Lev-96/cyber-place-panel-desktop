  ...
  return (
    <Modal open onClose={onClose}>
      <form className="card" style={{ width: 420, maxWidth: "90vw", display: "flex", flexDirection: "column", gap: 14 }} onSubmit={submit}>
        <h2 style={{ margin: 0 }}>{t("booking.rateBranchTitle")}</h2>
        <div className="col" style={{ gap: 6 }}>
          <span className="label">{t("booking.stars")}</span>
        ...
        <Input label={t("booking.commentOpt")} value={comment} onChange={(e) => setComment(e.target.value)} />
        {err && <div className="error">{err}</div>}
        <div className="row-between">
          <Button type="button" variant="secondary" onClick={onClose} disabled={busy}>{t("action.cancel")}</Button>
          <Button disabled={busy}>{busy ? t("booking.sending") : t("booking.submitRating")}</Button>
        </div>
      </form>
    </Modal>
  );
