  ...
  return (
    <div className="col" style={{ gap: 18 }}>
      <div className="row-between">
        <h2 className="page-title" style={{ margin: 0 }}>Products · branch #{id}</h2>
      ...
      {!loading && !error && (
        <div className="list">
          {(data ?? []).map((p) => (
            <div key={p.id} className="list-item" style={{ opacity: p.is_active ? 1 : 0.5 }}>
              <div>
                ...
                <div className="meta">{p.category ?? "—"} · {Number(p.price).toFixed(2)}</div>
              </div>
              <div className="row" style={{ gap: 6 }}>
                <Button variant="secondary" onClick={() => toggle(p)} style={btn}>{p.is_active ? "Hide" : "Show"}</Button>
          ...
          {!data?.length && <div className="muted">No products yet.</div>}
        </div>
      )}
