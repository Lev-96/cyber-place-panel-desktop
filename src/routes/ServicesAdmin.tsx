...
const ServicesAdmin = () => {
  ...
  return (
    <ScreenWithBg bg="./bg/admin-home.jpg" title="Services">
      <div className="row-between">
        <span className="muted">Global services available for branches to enable.</span>
          ...
          {(data ?? []).map((s) => (
            <div key={s.id} className="list-item">
              ...
              <div className="row" style={{ gap: 6 }}>
                <Button variant="secondary" onClick={() => setEditing(s)} style={btn}>Edit</Button>
                <Button variant="secondary" onClick={() => remove(s)} style={{ ...btn, color: "#ef4444", borderColor: "#4a1a1a" }}>Delete</Button>
              </div>
            </div>
          ))}
          {!data?.length && <div className="muted">No services.</div>}
        </div>
      )}
