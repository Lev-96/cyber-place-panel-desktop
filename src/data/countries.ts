/**
 * ISO 3166-1 country list used by the company-creation country picker.
 *
 * Russia (RU), Azerbaijan (AZ) and Turkey (TR) are intentionally EXCLUDED
 * by product decision (2026-05-31) — every other country is available.
 *
 * Each entry carries the international dialling code so the phone field can
 * be prefilled on selection, and the 2-letter code drives per-country TIN
 * validation (see ./tin.ts). Flag emoji is derived from the code at render
 * time (no need to store it).
 */
export interface Country {
  ...
  name: string;
  /** International dialling code, digits only (no leading "+"). */
  dial: string;
}

/** Codes excluded from the picker by product decision. */
export const EXCLUDED_COUNTRY_CODES = ["RU", "AZ", "TR"] as const;
...
/** Pickable countries — full ISO list minus the excluded codes, A→Z by name. */
export const COUNTRIES: Country[] = ALL_COUNTRIES
  .filter((c) => !EXCLUDED_COUNTRY_CODES.includes(c.code as (typeof EXCLUDED_COUNTRY_CODES)[number]))
...
export const countryByCode = (code: string | null | undefined): Country | undefined =>
  code ? BY_CODE[code.toUpperCase()] : undefined;

/** Regional-indicator flag emoji from an alpha-2 code, e.g. "AM" → 🇦🇲. */
...
/** Best-effort reverse lookup so editing a legacy free-text country still
 *  pre-selects the dropdown. Matches by code first, then by English name. */
export const resolveCountryCode = (value: string | null | undefined): string => {
  if (!value) return "";
  const v = value.trim();
